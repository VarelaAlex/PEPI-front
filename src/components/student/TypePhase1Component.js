				...
				if ( text.length === i.value.length ) {
					if ( normalize(text.toLowerCase()) === normalize(i.value.toLowerCase()) ) {
						...
						if ( current === 0 || current === 4 ) {
							setTimeout(() => {
								setExtendedNodes(extendedNodes.map(node => node.type === "type1" ? { ...node, clicked: false, ok: false } : node));
							}, 500);
						}
						element.ok = true;
						i.readOnly = true;
						...
						if ( element.shape ) {
							setFeedback({
								            phase1: {
									            ...feedback.phase1, lexicalError: feedback?.phase1?.lexicalError == null ? 1 : feedback?.phase1?.lexicalError + 1
								            }
							            });
						} else if ( element.stop || element.bigStop ) {
							setFeedback({
								            phase1: {
									            ...feedback.phase1, syntacticError: feedback?.phase1?.syntacticError == null ? 1 : feedback?.phase1?.syntacticError + 1
								            }
							            });
						} else {
							setFeedback({
								            phase1: {
									            ...feedback.phase1, semanticError: feedback?.phase1?.semanticError == null ? 1 : feedback?.phase1?.semanticError + 1
								            }
							            });
						}
					}
										...
										<rect
											...
											onClick={ () => {
												if ( element.order === current ) {
													...
													setFeedback({
														            phase1: {
															            ...feedback.phase1, orderError: feedback?.phase1?.orderError == null ? 1 : feedback?.phase1?.orderError + 1
														            }
													            });
												}
											} }
